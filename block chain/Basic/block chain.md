...
- [白皮书概念整理](#白皮书概念整理)
  ...
  - [3. 交易脚本](#3-交易脚本)
  - [4. 区块](#4-区块)
  - [5. 时间戳服务器（Timestamp server）](#5-时间戳服务器timestamp-server)
  - [6. 工作量证明（Proof-of-Work）](#6-工作量证明proof-of-work)
  - [7. 运行机制](#7-运行机制)
- [《精通比特币》相关要点整理](#精通比特币相关要点整理)
  - [1. 比特币交易介绍](#1-比特币交易介绍)
    - [1.1 获取正确的输入](#11-获取正确的输入)
    - [1.2 创建交易输出](#12-创建交易输出)
    - [1.3 将交易放到总账簿中](#13-将交易放到总账簿中)
  - [2. 比特币地址](#2-比特币地址)
    - [2.1 私钥格式](#21-私钥格式)
    - [2.2 公钥格式](#22-公钥格式)
    - [**2.3 生成地址**](#23-生成地址)
  - [3. 钱包](#3-钱包)
    - [3.1 创建助记词](#31-创建助记词)
    - [3.2 从助记词生成根种子](#32-从助记词生成根种子)
    - [3.3 从根种子中创造HD钱包](#33-从根种子中创造hd钱包)
  - [4. 比特币交易在网络中的传播](#4-比特币交易在网络中的传播)
  - [5. 交易结构](#5-交易结构)
  - [6. 交易的输出和输入](#6-交易的输出和输入)
    - [6.1 交易输出](#61-交易输出)
    - [6.2 交易输入](#62-交易输入)
  - [7. 交易脚本](#7-交易脚本)
    - [7.1 标准交易类型](#71-标准交易类型)
    - [7.2 多重签名](#72-多重签名)
...
- [分类](#分类)
  ...
  - [侧链](#侧链)
- [问题](#问题)
  - [比特币中的区块记录的交易账单是什么？这些交易账单由谁产生？十分钟才能确认，那么十分钟内不是得等着？](#比特币中的区块记录的交易账单是什么这些交易账单由谁产生十分钟才能确认那么十分钟内不是得等着)
  - [一个区块包含多少交易账单？为什么区块每十分钟产生一个，由谁控制？](#一个区块包含多少交易账单为什么区块每十分钟产生一个由谁控制)
  - [当前区块没有被矿工挖出来怎么办？](#当前区块没有被矿工挖出来怎么办)
  - [两个节点同时广播不同记录，链分叉了如何处理，具体的？](#两个节点同时广播不同记录链分叉了如何处理具体的)
  - [★结点下线一段时间后再上线，如何同步数据？直接copy过来么？结点下线后是否影响算力呢？如果大部分节点下线，那么当前全网算力下降，是否更容易被攻击篡改呢？](#结点下线一段时间后再上线如何同步数据直接copy过来么结点下线后是否影响算力呢如果大部分节点下线那么当前全网算力下降是否更容易被攻击篡改呢)
  - [区块结构到底是什么样子的？只有merkle tree的root么？](#区块结构到底是什么样子的只有merkle-tree的root么)
