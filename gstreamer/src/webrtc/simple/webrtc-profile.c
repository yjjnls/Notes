...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  receiver_entry->pipeline = gst_parse_launch ("webrtcbin name=webrtcbin "
      "videotestsrc pattern=ball ! openh264enc gop-size=20 ! h264parse ! "
      "video/x-h264,stream-format=avc,alignment=au,profile=baseline ! queue ! "
      "rtph264pay config-interval=-1 name=payloader ! queue ! "
